...
进程模型：无常驻 daemon。CLI 直接调用 core 库执行；`skillkit serve` 启动 Axum web server，同样直接调用 core 库。状态实时性由 core 写状态文件 + web 端 SSE 推送保证，不引入 daemon 生命周期管理。CLI 与 web server 并发写同一配置时，用文件锁（`~/.skm/` 下的写时锁）避免冲突。
...
```
<project>/
  .claude/skills/<skill-name>/        # shared 真实文件（git 提交，skillkit 只读发现）
  .claude/skills/local/<skill-name>   # symlink → ~/.skm/skills/<skill-name>/（gitignore）
  .cursor/skills/<skill-name>/        # shared 真实文件（git 提交，只读发现）
  .cursor/skills/local/<skill-name>/  # copy 自 ~/.skm/skills/<skill-name>/（gitignore）
  .codex/skills/...                   # 同理
```

skillkit 不在项目目录写入自己的配置文件（项目元数据全部放在 `~/.skm/projects/<project-id>.toml`，保持 local 配置个人本地、不入库）。唯一会改动的是项目原生的 `.gitignore`：`skillkit project add` 或 `apply` 自动追加 local 目录的忽略规则（若缺失），内容如下：
...
|-------|:---:|:---:|------|------|
| Claude Code | 否 | 是 | symlink `~/.claude/skills/<skill>` → `~/.agents/skills/<skill>` | symlink `<project>/.claude/skills/local/<skill>` → `~/.skm/skills/<skill>` |
| Cursor | 是 | 否 | 无需操作（直读 `~/.agents/skills/`） | copy `~/.skm/skills/<skill>` → `<project>/.cursor/skills/local/<skill>/` |
| OpenCode / Codex / Gemini | 是 | 是 | 无需操作（直读） | symlink 或 copy 均可，默认 symlink |

agent 列表和能力在 `~/.skm/config.toml` 声明，新增 agent 只改配置不改代码。Cursor 因不支持 symlink，项目 local skill 用 copy 兜底，apply 时按 canonical 内嵌的 commit_sha 检测副本是否过期，过期则重新 copy。
...
- `locked_shas`：项目对各 skill 锁定的 commit_sha，用于版本冲突检测。
...
`skillkit project apply <project>` 是核心命令，让项目的 `<agent>/skills/local/` 目录与 `installed_skills` 声明一致。流程幂等，可重复执行无副作用。
...
2. 计算目标状态：每个 agent 应有哪些 skill 落地
  │
  ▼
3. 扫描现状：<project>/<agent>/skills/local/ 下现有的 symlink 和目录
  │
  ▼
4. diff 并幂等执行：
   ├─ 该有但没有 → 建立（支持 symlink 的 agent 建 symlink，Cursor 等 copy）
   ├─ 不该有但有 → 删除（symlink 删链接，copy 删目录）
   └─ 版本冲突 → 默认以 canonical 为准更新 locked_shas；--frozen 模式报错不动
  │
  ▼
5. scope=global 的 skill 落地是全局的（确保 ~/.agents/skills/ 存在 + ~/.claude/skills/ symlink），
...
单版本模型下，canonical 物理存储只有一份。冲突检测保证升级时不会默默让某个锁定旧版本的项目失控。
...
skillkit project scan <dir> [--depth 3]            # 扫描目录发现项目
...
| CLI 与 web server 并发写 | registry/projects 文件写时锁，避免写写冲突 |
| 项目目录无写权限 | 报错退出，不降级静默 |
...
- 扫描导入现有 skill（`~/.agents/skills/` 和 `~/.claude/skills/`，能确定源的标记版本，手动放的标记 unmanaged，GUI 里归类到 profile）。
- 版本锁和冲突检测完善。
- Cursor copy 一致性保证。
